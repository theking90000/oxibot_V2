...
const LoadLang = async (lang) => {
        ...
        window.localStorage.setItem(`oxibotV2_locales_${lang}`, JSON.stringify({translation : response.translates}));
    
}

export var AvailableRessources :{lang : string,langname : string}[]= []

export const setup = async (defaultlang : string,langs : {lang : string,langname : string}[]) => {
