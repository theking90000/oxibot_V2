...
export const getUser = async (userID: string): Promise<IUsersWebDocument> => {
  const user: IUsersWebDocument = usersweb.has(userID)
    ? usersweb.get(userID)
    : null;
...
export const selectUser = async (userID: string): Promise<ActionUserWeb> => {
  ...
  let guildx = (await client.guilds.cache)
    .filter((g) => g.members.cache.has(user.DiscordUserID))
    .map((x) => x);
  ...
  for (const guild of guildx) {
    try {
      const selectedUP = permission.getUser({
        guildID: guild.id,
...
export const updateUserToken = async (g_: createGroupT) => {
  ...
  (userweb.token = g_.token),
    ...
    (userweb.expires =
      Math.round(date.getTime() / 1000) + parseInt(g_.expires));
  await userweb.save();
